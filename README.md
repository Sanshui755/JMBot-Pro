...
- **超分辨率下载**：`/jm -h <id>` 或 `/jm <id> -h`，使用 Real-ESRGAN（ncnn-vulkan，anime 模型）对下载图片进行 4x 超分辨率后再合并 PDF，画质显著提升；首次使用时自动从 GitHub 下载约 45MB 的工具包，无需安装 torch 等重型依赖
- **站内搜索**：`/jms <关键词>` 按关键词搜索本子，返回前 10 条结果（车号、标题、标签），兼容 `/jms关键词` 无空格写法
- **作者搜索**：`/jma <作者名>` 按作者搜索本子，返回前 10 条结果，兼容 `/jma作者名` 无空格写法
...
| `/jm -h <id>` 或 `/jm <id> -h`      | 超分辨率下载（Real-ESRGAN anime 模型 4x 放大，首次自动下载工具包） | 群聊需先开启，私聊默认可用 |
| `/jm help`                          | 查看使用帮助                                                       | 所有人                     |
| `/jms <关键词>`                     | 站内搜索本子（前 10 条结果），兼容 `/jms关键词` 无空格写法         | 群聊需先开启，私聊默认可用 |
| `/jma <作者名>`                     | 按作者搜索本子（前 10 条结果），兼容 `/jma作者名` 无空格写法       | 群聊需先开启，私聊默认可用 |
...
>
> 方法一（AstrBot 插件）的下载与管理命令与上表一致，区别仅为超管 / 加密 / 下载路径等配置优先在 WebUI 中完成；`/jms`、`/jma`、`/jmv`、`/jm -h`（超分辨率）目前为方法一（AstrBot 插件）独有。
