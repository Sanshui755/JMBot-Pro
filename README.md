...
- **超分辨率下载（双模型可选）**：对下载图片进行超分辨率放大后再合并 PDF，画质显著提升；两套工具均为 ncnn-vulkan 预编译二进制，首次使用时自动从 GitHub 下载，无需安装 torch 等重型依赖
  - `/jm -h <id>` 或 `/jm -hr <id>`：Real-ESRGAN（anime 模型，4x 放大，工具包约 45MB）
  - `/jm -hw <id>`：waifu2x（cunet 动漫模型，降噪级别 2，2x 放大，工具包约 35MB）
...
| `/jm -h <id>` 或 `/jm -hr <id>`     | 超分辨率下载（Real-ESRGAN anime 模型 4x，首次自动下载约 45MB 工具包） | 群聊需先开启，私聊默认可用 |
